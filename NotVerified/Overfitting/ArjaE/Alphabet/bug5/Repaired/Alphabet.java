public class Alphabet {
   ...
   public boolean[] driver(int op) 
   {
	...
	if (!(0 < result.length && 1 < result.length && 0 < result.length && 2 < result.length && 0 < result.length && 3 < result.length && 0 < result.length && 4 < result.length && 0 < result.length && 5 < result.length)) return result;
	switch (op) {
		case 0 :
		result[0] = isVowel();
